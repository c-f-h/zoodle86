...
There is no separate unit-test suite. A successful build is the current baseline check. Invoking the `/bin/hello 3 3` (PID, spawning, basic syscalls), `/bin/fs_stress` (syscall-heavy file operations), and `/bin/alloc_stress` (userspace memory manipulation) programs using the `run` command from the kernel shell can serve as additional checks.
