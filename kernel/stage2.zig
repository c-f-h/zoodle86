...
fn loadKernelElfAndJump() noreturn {
    const root_inode = disk_fs.readRootInode() catch
        earlyBootFail("FS error: cannot read root");
    const kernel_inode = disk_fs.findDirEntryInodeReadOnly(&root_inode, "kernel") catch
