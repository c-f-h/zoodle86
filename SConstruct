...
STAGE2_INIT_ASM = ROOT / "kernel" / "stage2_init.asm"
...
def assemble_stage2_init(target, source, env):
    ...
    run([str(NASM_EXE), "-f", "elf32", str(STAGE2_INIT_ASM), "-o", str(STAGE2_INIT_OBJ)])
    if not STAGE2_INIT_OBJ.exists():
        raise RuntimeError("NASM did not produce stage2_init.o.")
