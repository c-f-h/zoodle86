...
Each `Task` struct starts with an 8 KiB kernel stack region (its first word stores a pointer back to the `Task` for `getCurrentTask()`), followed by a per-task page directory, a unique PID, a saved `kernel_esp`, an owned canonical current-working-directory string, user-mode stack/heap bounds, a small per-task fd table, an optional controlling tty, and a `waiters_for_pid` WaitQueue used by `waitpid`. Newly created tasks seed two stack artifacts in that kernel-stack region: a persistent `UserInterruptFrame` at the top, plus a lower synthetic `KernelFrame` that returns into the `return_to_userspace` trampoline the first time the task is scheduled. Once a task is actively running, `kernel_esp` is set to 0; when the task blocks or is preempted, it is updated to the exact kernel stack position where `kernel_yield()` should resume it. When a task is bound to a tty, fd slots 0-2 are populated with stdin/stdout/stderr mappings to that tty. Each task also carries a `TaskState` (`.free`, `.active`, `.waiting`, `.zombie`), a `parent_pid` (0 for shell-spawned tasks), a `reap_children` flag, and an `exit_status`.

User-mode execution uses flat-addressed segments backed by the user memory region. The kernel can load and execute freestanding ELF binaries (ELF32 format) by extracting code and data segments, computing heap and stack boundaries (page-aligned above the program image), and mapping those segments into GDT selectors 0x1B and 0x23 (user code/data, DPL=3). `taskman.zig` allocates a fixed pool of 8 `TaskmanEntry` slots at runtime, each laid out as `[8 KiB guard region][Task]`.

## Argv ABI

Command-line arguments are passed to a new process via `Task.setArgs(args)` and written into the userspace stack. `_start` (naked) pushes ESP and calls `argvStartup`, which reconstructs `[]const []const u8` from the ABI layout and passes it to `main(argv)`. The `run <executable> [<arg>...]` shell command launches one program with arguments (argv[0] = executable name); `multirun <count> <executable> [<arg>...]` launches `count` copies of the same program with the same argv. `spawn` resolves relative executable paths against the parent's current working directory and copies that cwd into the child.
