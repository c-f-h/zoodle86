...
| `ioctl` | 156 | fd, command, arg | device-specific | Currently supports tty mode switching via `IOCTL_TTY_SET_MODE` with `TTY_MODE_CANONICAL`/`TTY_MODE_RAW`; tty mode switches return the previous tty mode. |
...
| `framebuf` | 1005 | framebuf_info_out | 0 | Fills a writable userspace `FrameBufInfo` struct and maps the boot framebuffer into a fixed userspace window for direct pixel access; returns `ENODEV` if no linear framebuffer is available. |
