...
| `kshell` | 1003 | cmdline_slice_ptr | 0 or `FAIL` | Executes a kernel shell command string. `cmdline_slice_ptr` is a userspace address pointing to an `AbiSlice` describing the command string. Uses the calling task's console or the primary console as fallback. |
