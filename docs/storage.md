...
The build pipeline assembles the root directory from `build/fsimage/`, which is populated with the kernel module, userspace binaries, optional `autoexec`, and every regular file from `static/`.
